...
#define SWEEP_SPEED 2.0f //稍慢閃少一點. default 3.0f 
...
static void draw_radar() {
    ...
    static uint32_t last_blip_draw = 0;
    uint32_t now = millis();
	...
	if (redraw_blips) { 	
		...
		for (int i = 0; i < prev_blip_count; i++) {
			...
			/*	3.3 標籤半截字 + 重疊（核心 UI 優化）
				問題:
				擦除固定 fillRect(..., 56, 18)，字較長或兩行時擦不乾淨 → 殘影／半截感。
				幾乎每架 behind < 180 都畫 callsign，忙區重疊。
				靠近螢幕邊緣字畫出去。
			*/
			...
			for (int i = 0; i < aircraft_list.count && new_blip_count < MAX_AIRCRAFT; i++) {
				...
				*/
				if (a.is_emergency)
					...
					fade_color = pal->blip;   //pal->fade_bright也可,一樣亮
				...
				b.has_label = false;
				if (labels_drawn < MAX_LABELS) {  //if (behind < 90 && labels_drawn < MAX_LABELS) {
					// 邊界檢查：太貼邊就不畫字，避免 half-cut
					//    if (px > 8 && px < LCD_H_RES - 70 && py > RADAR_Y + 14 && py < LCD_V_RES - 14) {
...
void setup() {
    ...
    // 不上電白光會洗掉螢幕（與你 working demo 的 cydRgbLedOff 相同）
    pinMode(4, OUTPUT);   // R
    pinMode(16, OUTPUT);  // G
    pinMode(17, OUTPUT);  // B
    digitalWrite(4, HIGH);
